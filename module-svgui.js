define('svgui', ['ui', 'svg'], function (ui, svg) {
	...
	var svgui = {
		...
		target: null,
		fields: ['x','y','x1','y1','x2','y2','cx','cy','r','width','height','stroke','stroke-width','fill','style','index'],
	...
	svgui.newField = function(name, type){
		...
		label.className = label.innerHTML = field.name = field.id = field.className = name;
		field.type = type;
		field.value = '';
	...
	svgui.updateui = function(e){
		if(e.target.tagName != 'svg' && svg.mouse.mode.name == 'click'){
			...
			// Disable all fields
			var fields = this.form.querySelectorAll('label:not(.index),input:not(.index)');
			...
			this.el.childNodes.forEach(function(item, index, list){
				if(item == this.target){
